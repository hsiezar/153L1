...
exit(int status)
{
  ...
  struct proc *p;
  int fd;
  curproc->exitStatus = status;//LAB1 CS153 ADDED
...
scheduler(void)
{
  ...
  for(;;){
	...
	smallPrior = 0;
      ...
      if(smallPrior != 0) {
      c->proc = smallPrior;
      switchuvm(smallPrior);
      smallPrior->state = RUNNING;
      smallPrior->priority -= 2;
      swtch(&(c->scheduler), smallPrior->context);
      switchkvm();
